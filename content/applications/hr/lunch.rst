...
.. tip::
   ...
   Note that for many forms, the :guilabel:`Save` button is not required, and may not appear.
...
The availability section presents a table with two rows. The days of the week populate the top row,
and the bottom row has checkboxes. Check the corresponding box for each day of the week the vendor
is available.
...
- :guilabel:`Company`: if the product should only be available to a specific company, select it from
  ...
  the database.
- :guilabel:`Image`: hover over the image box in the top-right, and click the :guilabel:`✏️
